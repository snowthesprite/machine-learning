...
class RandDecisionTree () :
    ...
    def make_tree(self) :
        ...
        while queue != [] :
            ...
            split = self.find_rand_split(node.points)
            if node.entropy == 0 or len(node.points) < self.min_to_split or split == {} :
                node.type = self.find_node_type(node)
                continue
                
            node.split = split['split']
            great_node = SplitNode(split['greater'], 'grt' + str(node.split))
            less_node = SplitNode(split['lesser'],'less' + str(node.split))
    ...
    def find_rand_split(self, points) :
        ...
        axis = math.floor(random.random()*self.dim)
        #print(splits)
        while len(splits[axis]) == 0 :
            axis = math.floor(random.random()*self.dim)
        split = list(splits[axis])[math.floor(random.random()*len(splits[axis]))]
        #print(split)
        greater = [point for point in points if point['coord'][axis] >= split]
        lesser = [point for point in points if point['coord'][axis] < split]
        return {'split': (axis,split), 'greater': greater, 'lesser': lesser}
