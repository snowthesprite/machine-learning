...
for row_id in range(book_type.size) :
    ...
    for k in range(1,100) :
        ...
        KNN_simp = KNeighborsClassifier(n_neighbors=k)
        KNN_simp.fit(mod_df_simp, mod_book_type)
        predict_simp[k].append(KNN_simp.predict([df_simple.iloc[row_id]]))

        KNN_min = KNeighborsClassifier(n_neighbors=k)
        KNN_min.fit(mod_df_min, mod_book_type)
        predict_min_max[k].append(KNN_min.predict([df_min_max.iloc[row_id]]))

        KNN_z = KNeighborsClassifier(n_neighbors=k)
        KNN_z.fit(mod_df_z, mod_book_type)
        predict_z[k].append(KNN_z.predict([df_z.iloc[row_id]]))
    if row_id % 10 == 0 :
        print('k', k)
...
plt.savefig('analysis/KNN/KNN_model_.png')
